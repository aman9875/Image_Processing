		...
		if (numObjects < MAX_NUM_OBJECTS) {
			...
			if (objectFound == true) {
				...
				if (flag == 1)
				{
					...
					float ball_area = minEnclosingCircle(contours[maxindex], center, radius).area();
				...
				else if (flag == 0)
				{
					...
					cout << distance << "\n";
					rectangle(cameraFeed, boundingRect(contours[maxindex]).tl(), boundingRect(contours[maxindex]).br(), color, 2, 8, 0);
					//drawObject(x, y, cameraFeed, center, radius);
				}
			}
